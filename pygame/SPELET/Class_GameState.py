...
class GameState():
    ...
    def Titlecard(self):
        ...
        text_obj = font_obj.render("Davids Äventyr",True,White)
        text_rect = text_obj.get_rect(center = (screen_Width//2, screen_Height//2))

        font_obj2 = pygame.font.Font("Fonts/Font1.TTF",int(31*scale))
        text_obj2 = font_obj2.render("Press spacebar to continue", True, White)
    ...
    def Difficulty_Scene(self):
        ...
        screen.blit(text_obj4,((z/(3+1)+image_Width*0.8*0.1, 500*scale)))
        screen.blit(text_obj5,((2*(z)/(3+1)+image_Width*0.8+image_Width*scale*0.1),500*scale))
        screen.blit(text_obj6,((3*(z)/(3+1)+2*image_Width*0.8+image_Width*0.8*0.1), 500*scale))
        pygame.display.flip()
    ...
    def Chest_Scene_Open(self):
        ...
        screen.blit(item, (self.screen_Width//2-item_Width/2, 500*scale))
        Continue_Button.draw(screen)
